...
class Cms::Public::Node::PagesController < Cms::Controller::Public::Base
  def index
    ...
    if request.mobile?
      Page.title = @item.mobile_title unless @item.mobile_title.blank?
      @body = @item.mobile_body unless @item.mobile_body.blank?
    end
  end
  ...
  def render_public_variables
    response.body.scan(/\{\$[a-z]+\}/i).uniq.each do |name|
      ...
      response.body = response.body.gsub(name.to_s, value) if name != value
    end
  end
end
