...
module Cms::Model::Rel::EmbeddedFile
  ...
  module ClassMethods
    def embed_file_of(*names)
      ...
      self.embedded_file_names.each do |name|
        belongs_to "embedded_file_relation_of_#{name}",
                   foreign_key: name,
  ...
  def embedded_file_site_id
    return site_id if respond_to?(:site_id)
    return content.site_id if respond_to?(:content)
    nil
  end
  ...
  def embedded_file_option(name, type)
    ...
    if self.class.embedded_file_options[name].key?(type)
  ...
  def save_embedded_files
    ...
    names.each do |name|
      ...
      send("#{name}=", item.id) if item.save
    end
    true
  end
end
