...
class Cms::Lib::Navi::Jtalk
  def self.make_text(html)
    ...
    html.gsub!(/.*<body.*?>(.*)<\/body.*/i, '\\1') if html =~ /<body/i
    if html =~ /<!-- skip.reading -->/
      html.gsub!(/<!-- skip.reading -->.*?<!-- \/skip.reading -->/i, '')
    end
    %w(style script noscript iframe rb rp).each do |name|
    ...
    mc.parse(html).split(/\n/).each_with_index do |line, _line_no|
      p = line.split(/,/)
      next if line == 'EOS'
  ...
  def make(*args)
    ...
    texts.each do |text|
      ...
      parts << wav if FileTest.exists?(wav.path)
      FileUtils.rm(cnf.path) if FileTest.exists?(cnf.path)
    end

    wav = Tempfile.new(['talk', '.wav'], '/tmp')
    mp3 = Tempfile.new(%w(talk mp3), '/tmp')

    cmd = "#{sox} #{parts.collect(&:path).join(' ')} #{wav.path}"
    system(cmd)
