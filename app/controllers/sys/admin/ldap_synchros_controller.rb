...
class Sys::Admin::LdapSynchrosController < Cms::Controller::Admin::Base
  ...
  def do_synchro(group, parent = nil)
    ...
    if sg.ldap == 1
      ...
      sg.web_state ||= 'public'
      sg.name        = group.name
      sg.name_en     = group.name_en unless group.name_en.blank?
      sg.email       = group.email unless group.email.blank?
    ...
    ## next
    group.children.each { |g| do_synchro(g, sg) } if group.children.size > 0
  end
  ...
  def create_synchros(entry = nil, group_id = nil)
    ...
    group = Sys::LdapSynchro.new(parent_id: group_id,
                                 version: @version,
                                 entry_type: 'group',
                                 code: entry.code,
                                 name: entry.name,
    ...
    entry.users.each do |e|
      user = Sys::LdapSynchro.new(parent_id: group.id,
                                  version: @version,
                                  entry_type: 'user',
                                  code: e.uid,
                                  name: e.name,
