...
module EntityConversion::Lib::Convertor::Test
  ...
  def convert_move(unit, group)
    if @end_ids[group.id]
      ...
      raise "group is not exists #{group.name} > #{move.name}" unless @new_codes[move.code]
    else
  ...
  def convert_end(_unit, group)
    raise "group is not exists #{group.name}" if @end_ids[group.id]
  ...
  def replace_group_id_save(item, fields, old_id, new_id)
    fields.each do |field|
      ...
      if value.is_a?(Fixnum)
        ...
        value = value.gsub(/((^| )#{old_id}( |$))/, '\\2' + new_id.to_s + '\\3')
      end
