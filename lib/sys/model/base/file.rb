...
module Sys::Model::Base::File
  ...
  def self.sizes(options = {})
    sizes = []
    sizes << [(options[:include_blank]).to_s, ''] if options.key?(:include_blank)
  ...
  def upload_path(options = {})
    md_dir  = self.class.to_s.underscore.pluralize.to_s
    id_dir  = format('%08d', id).gsub(/(.*)(..)(..)(..)$/, '\1/\2/\3/\4/\1\2\3\4')
    id_file = format('%07d', id) + '.dat'

    id_file = "#{options[:type]}.dat" if options[:type]
  ...
  def css_class
    if ext = File.extname(name).downcase[1..5]
      ...
      ext = ext.gsub(/[^0-9a-z]/, '')
      return 'iconFile icon' + ext.gsub(/\b\w/, &:upcase)
    end
    'iconFile'
  end
  ...
  def reduce_size(options = {})
    ...
    return "width: #{dst_w.ceil}px; height:#{dst_h.ceil}px;" if options[:css]
