...
module Storage::Db
  ...
  def self.find(path)
    find_records(path).collect(&:path)
  end

  def self.entries(path)
    entry_records(path).collect(&:basename)
  end
  ...
  def self.rm_rf(path)
    find_records(path).each(&:destroy) if directory?(path)
    storage(path, &:destroy)
    true
  end
