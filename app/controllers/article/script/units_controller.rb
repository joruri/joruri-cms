  ...
  def publish
    ...
    Article::Unit.root_item.public_children.each do |item|
      ...
      attrs.each do |attr|
        ...
        publish_more(item, :rel_unid => attr.unid, :uri => uri, :path => path, :dependent => "/#{attr.name}")
      end
      ...
      item.public_children.each do |c|
        ...
        publish_more(c, :uri => uri, :path => path, :file => 'more', :dependent => :more)
        ...
        attrs.each do |attr|
          ...
          publish_more(c, :rel_unid => attr.unid, :uri => uri, :path => path, :dependent => "/#{attr.name}")
        end
      end
    end
