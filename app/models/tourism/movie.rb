# encoding: utf-8
class Tourism::Movie < ActiveRecord::Base
  ...
  validates_presence_of :state, :title, :uri, :genre_id, :spot_id
  ...
  def public_path
    ...
    "#{content.public_path}/movies/#{_name}/index.html"
  end
  ...
  def public_uri
    ...
    return nil unless node = content.movie_node
  ...
  def youtube_key
    ...
    if uri =~ /\.?youtube\.com\/.*\?v=[\w]+(&|$)/i
      return uri.gsub(/.*\?v=([\w]+)(&.*|$)/i, '\\1')
    elsif uri =~ /youtu\.be\/[\w]+(\?|$)/i
      return uri.gsub(/.*youtu\.be\/([\w]+)(\?.*|$)/i, '\\1')
    end
    nil
  end
  ...
  def thumbnail_uri
    key = youtube_key
    key ? "http://i.ytimg.com/vi/#{key}/default.jpg" : nil
  end

  def embedded_html
    if key = youtube_key
      uri = "http://www.youtube.com/embed/#{key}"
      # return %Q(<iframe width="420" height="315" src="#{uri}" frameborder="0" allowfullscreen></iframe>)
      # return %Q(<iframe width="720" height="405" src="#{uri}" frameborder="0" allowfullscreen></iframe>)
      return %(<iframe width="640" height="480" src="#{uri}" frameborder="0" allowfullscreen></iframe>)
    end
    nil
  end
  ...
  def search(params)
    params.each do |n, v|
      ...
      when 's_spot'
        ...
        ids = item.find(:all, select: :id).collect(&:id)
