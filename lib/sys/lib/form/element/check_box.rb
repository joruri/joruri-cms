...
class Sys::Lib::Form::Element::CheckBox < Sys::Lib::Form::Element::Base
  ...
  def make_tag
    values = @value.blank? ? [] : value.collect { |_k, v| v }.uniq

    tag = %(<div #{attributes_string}>)
    select_options.each_with_index do |v, k|
      ...
      tag += %(<label for="#{tag_id(k)}">#{v}</label>)
      tag += '</div>'
    end
    tag += '</div>'
    tag.html_safe
  ...
  def make_frozen_value
    return '' if @value.blank?
    values = value.collect { |_k, v| v }.uniq

    tag = ''
    select_options.each_with_index do |v, _k|
      if value.index(v)
