...
class Cms::Admin::PreviewController < Cms::Controller::Admin::Base
  ...
  def replace_preview_data
    ...
    ## preview mark
    html = %(<div id="cmsPreviewMark") +
           %( style="position: absolute; top: 0px; left: 0px; width: 80px; margin: 1px; padding: 2px; ) +
           %(   border: 1px solid #777; background-color: #dfd; ) +
           %(   line-height: 1.5; font-family: sans-serif; text-align: center; cursor: pointer; ") +
           %( onclick="document.getElementById('cmsPreviewMark').style.display='none';" ) +
           %(>プレビュー</div>)
