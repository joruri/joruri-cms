...
class Newsletter::Script::RequestsController < ApplicationController
  ...
  def subscribe(req)
    ...
    mem = Newsletter::Member.new(state: 'enabled',
                                 content_id: req.content_id,
                                 letter_type: req.letter_type,
                                 email: req.email)
    raise mem.errors.full_messages.join(' ') unless mem.save

    send_mail(from: mail_from(req.content_id),
              to: mem.email,
  ...
  def unsubscribe(req)
    ...
    raise mem.errors.full_messages.join(' ') unless mem.destroy
