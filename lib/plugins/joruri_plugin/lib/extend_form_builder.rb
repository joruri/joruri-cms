...
class ActionView::Helpers::FormBuilder
  ...
  def array_select(method, choices, options = {}, _html_options = {})
    ...
    if options[:include_blank]
      ...
      choices = %(<option value="">#{label}</option>) + choices
  ...
  def radio_buttons(method, choices, options = {})
    ...
    choices.each do |label, value|
      ...
      h += %(<label for="#{@object_name}_#{method}_#{value}">#{label}</label>\n).html_safe
    end
    h.html_safe
  end

  def array_radio_buttons(method, choices, _options = {})
    value = array_value(method)
    method = array_name(method)
    ...
    choices.each do |c|
      ...
      id   = method.delete(']').tr('[', '_') + "_#{c[1]}"
      h += @template.radio_button_tag(method, c[1], (value.to_s == c[1]))
      h += %(<label for="#{id}">#{c[0]}</label>\n).html_safe
    end
    h.html_safe
  end

  def check_boxes(method, choices, _options = {})
    method = method.to_s

    checked = []
    if @template.params[@object_name] && @template.params[@object_name][method]
      @template.params[@object_name][method].each { |k, _v| checked << k }
    else
