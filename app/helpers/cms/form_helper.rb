...
module Cms::FormHelper
  ...
  def piece_replace_menu(item)
    if rep = item.replace_page
      %(<div class="noticeBox">更新用のピースが作成されています : #{link_to h(rep.title), rep.admin_uri}</div>).html_safe
    elsif org = item.replaced_page
      %(<div class="noticeBox">公開時に更新されるピース : #{link_to h(org.title), org.admin_uri}</div>).html_safe
    end
  end
end
