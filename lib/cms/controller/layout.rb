...
module Cms::Controller::Layout
  ...
  def render_public_as_string(path, options = {})
    return nil if path =~ /\.html\.r$/ && !Joruri.config[:cms_use_kana]
  ...
  def render_public_layout
    ...
    return http_error(404) if Page.ruby && !Joruri.config[:cms_use_kana]
    ...
    if Page.layout
      ...
      Page.layout = Cms::Layout.new(body: '[[content]]',
                                    mobile_body: '[[content]]',
    ...
    ## ruby(kana)
    body = Cms::Lib::Navi::Kana.convert(body) if Page.ruby
  ...
  def piece_container_html(piece, body)
    ...
    html = %(<div#{piece.css_attributes}>\n)
    html << %(<div class="pieceContainer">\n)
    html << %(<div class="pieceHeader"><h2>#{title}</h2></div>\n) unless title.blank?
    html << %(<div class="pieceBody">#{body}</div>\n)
    html << %(</div>\n)
    html << %(<!-- end .piece --></div>\n)
    html.html_safe
  end
